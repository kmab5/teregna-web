          ...
          <div className="grid gap-12 lg:grid-cols-[1.1fr_1fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Button asChild size="lg">
                  <Link href="/browse">
                    {t("landing.ctaFind")} <ArrowRight aria-hidden />
                  </Link>
                </Button>
                <Button asChild size="lg" variant="outline">
                  <Link href="/provider/login">{t("landing.ctaRun")}</Link>
                </Button>
              </div>
            </div>
            ...
            <div className="queue-rail relative space-y-3 rounded-[var(--radius-lg)] border border-border bg-surface p-4 elev-2 sm:p-6">
              <p className="mb-4 text-xs font-medium uppercase tracking-wide text-ink-muted">
                Abebe Barbershop · {t("landing.live")}
              </p>
              {[
        ...
        {featured.length > 0 ? (
          <section className="mx-auto max-w-6xl px-4 pb-16 sm:pb-20">
            <div className="mb-5 flex items-end justify-between">
              <h2 className="font-display text-xl font-semibold">{t("landing.openNow")}</h2>
              <Link
                href="/browse"
